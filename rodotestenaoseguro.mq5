...
//|        baixa)                                                      |
//|   - SL/TP dinâmicos baseados em ATR (Stop Loss reintroduzido       |
//|     após teste real mostrar Stop Out por margem insuficiente)      |
...
//|   - Prazo máximo de InpMaxHoldDays: fecha a mercado se o SL/TP     |
//|     ainda não tiverem sido atingidos (segurança extra, raramente   |
//|     deve disparar já que o SL deve agir primeiro)                  |
...
input double  InpATR_TrailStart     = 2.0;        // Lucro em múltiplos de ATR para ativar o trailing
input double  InpATR_TrailStep      = 1.5;        // Distância do trailing em múltiplos de ATR
...
void OpenPosition(ENUM_ORDER_TYPE orderType, double atrValue)
  {
   ...
   if(sent)
      Print("Posição aberta com SL em ", DoubleToString(sl, _Digits), " e TP em ", DoubleToString(tp, _Digits));
   else
