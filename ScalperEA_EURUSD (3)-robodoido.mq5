...
//|   - Lote calculado por % de risco do saldo da conta                |
//|   - Trailing stop baseado em ATR                                   |
//|   - Limite de perda diária (% do saldo) bloqueia novas entradas    |
//|   - Filtro de spread máximo                                        |
//|   - Limite de 1 trade por dia (sem exceção, independente do        |
//|     resultado do trade anterior)                                   |
...
input double  InpRiskPercent        = 10.0;       // % do saldo arriscado por operação
...
input group "=== Rotina Diária ==="
input bool    InpOneTradePerDay     = true;        // Limitar a 1 trade por dia (sem exceção)?
...
int      g_tradesToday = 0;          // contador de trades abertos hoje (limite de 1/dia)
...
//+------------------------------------------------------------------+
//| Verifica se o limite de 1 trade por dia já foi usado              |
//| (bloqueia sem exceção, independente do resultado do trade)        |
//+------------------------------------------------------------------+
bool IsOneTradePerDayBlocked()
  {
   if(!InpOneTradePerDay)
      return(false);
   return(g_tradesToday >= 1);
  }
...
void OpenPosition(ENUM_ORDER_TYPE orderType, double atrValue)
  {
   ...
   // Só consome a tentativa diária se a ordem foi de fato aceita pela corretora
   if(sent)
      g_tradesToday++;
   else
